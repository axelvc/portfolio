...
export default function Cursor() {
  ...
  return (
    <div ref={rootRef} className="fixed top-0 left-0 pointer-events-none -translate-x-1/2 -translate-y-1/2 scale-0">
      <div
        ref={dotRef}
        className="w-[24px] h-[24px] rounded-[24px] bg-green-600 opacity-30 transition-all duration-300"
      />
      <div
        ref={textRef}
        className="absolute top-1/2 left-1/2 grid place-items-center w-16 h-16 text-white font-medium capitalize -translate-x-1/2 -translate-y-1/2 opacity-0 scale-0 transition-all duration-300"
      />
    </div>
  )
}
